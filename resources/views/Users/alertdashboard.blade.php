<!doctype html>

<html lang="en">

@include('Users.components.head')

<body>
    <script src="./dist/js/demo-theme.min.js?1692870487"></script>
    <div class="page">

        @include('Users.components.aside')
        @include('Users.components.header')

        <div class="page-wrapper">
            <!-- Page header -->
            {{-- <div class="page-header d-print-none">
                <div class="container-xl">
                    <div class="row g-2 align-items-center">
                        <div class="col">
                            <!-- Page pre-title -->
                            <div class="page-pretitle">
                                Overview
                            </div>
                            <h2 class="page-title">
                                Dashboard
                            </h2>
                        </div>

                    </div>
                </div>
            </div> --}}
            <!-- Page body -->
            <div class="page-body py-4">
                <div class="container-xl">
                    <div class="row g-4">
                        <!-- Emergency Alert Section -->
                        <div class="col-12 col-lg-6">
                            <div class="card shadow-lg border-0">
                                <div class="card-body text-center">
                                    <div class="mb-3">
                                        <button class="btn btn-danger btn-lg w-100 rounded-pill fw-bold py-5"
                                            style="font-size: 2.5rem; box-shadow: 0 4px 15px rgba(255, 0, 0, 0.5); transition: 0.3s ease;">
                                            <span class="me-3">Alert</span>
                                            <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40"
                                                viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                                stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                                                class="icon icon-tabler icon-tabler-alert-hexagon">
                                                <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                                                <path
                                                    d="M19.875 6.27c.7 .398 1.13 1.143 1.125 1.948v7.284c0 .809 -.443 1.555 -1.158 1.948l-6.75 4.27a2.269 2.269 0 0 1 -2.184 0l-6.75 -4.27a2.225 2.225 0 0 1 -1.158 -1.948v-7.285c0 -.809 .443 -1.554 1.158 -1.947l6.75 -3.98a2.33 2.33 0 0 1 2.25 0l6.75 3.98h-.033z" />
                                                <path d="M12 8v4" />
                                                <path d="M12 16h.01" />
                                            </svg>
                                        </button>
                                    </div>
                                    <h5 class="fw-bold text-danger">Emergency Alert</h5>
                                    <p class="text-muted">Tap to notify responders immediately</p>
                                </div>
                            </div>
                        </div>

                        <!-- Help Request Section -->
                        <div class="col-12 col-lg-6">
                            <div class="card shadow-lg border-0">
                                <div class="card-body">
                                    <h5 class="fw-bold text-dark">Request Help</h5>
                                    <p class="text-muted">Select the type of emergency:</p>

                                    <!-- Emergency Type Dropdown -->
                                    <select class="form-select mb-3" id="emergencyType">
                                        <option selected disabled>Select an emergency type</option>
                                        <option value="medical">🚑 Medical</option>
                                        <option value="security">🔒 Security</option>
                                        <option value="fire">🔥 Fire</option>
                                    </select>

                                    <!-- Submit Help Request Button -->
                                    <div class="text-center">
                                        <button class="btn btn-success btn-md fw-bold w-100" onclick="sendHelpRequest()"
                                            style="box-shadow: 0 4px 10px rgba(0, 128, 0, 0.5);">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="icon me-2" width="24"
                                                height="24" viewBox="0 0 24 24" stroke-width="2"
                                                stroke="currentColor" fill="none" stroke-linecap="round"
                                                stroke-linejoin="round">
                                                <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                                                <path d="M10 3v18l7 -7l-7 -7z" />
                                            </svg>
                                            Send Help Request
                                        </button>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

        </div>
        @include('Users.components.footer')
    </div>
    </div>
    @include('Users.components.scripts')

</body>

</html>
